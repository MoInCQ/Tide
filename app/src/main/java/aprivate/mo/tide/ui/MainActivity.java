...
public class MainActivity extends BaseActivity<IMainActivityView, MainActivityPresenter>
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loadMultipleRootFragment(R.id.fl_main, HOME_FRAGMENT_FLAG, fragmentList.get(HOME_FRAGMENT_FLAG));
    ...
    public void initCitySelectInfo(final List<CitySelected> citySelectedList) {
        citySelectedLayoutManager = new GridLayoutManager(MainActivity.this, 3, OrientationHelper.VERTICAL, false);
        citySelectedAdapter = new DrawerSelectCityAdapter(citySelectedList);
        citySelectedAdapter.setOnItemClickListener(new DrawerSelectCityAdapter.OnCitySelectedClickListener() {
            @Override
            ...
            public void onItemLongClick(View view, int position) {
                Toast.makeText(MainActivity.this, "长按  " + citySelectedList.get(position).getName(), Toast.LENGTH_SHORT).show();
            }
        });
        ...
        switch (tab.getPosition()) {
            case EXPLORE_FRAGMENT_FLAG:
                if (!isExploreFragmentLoaded) {
                    loadMultipleRootFragment(R.id.fl_main, EXPLORE_FRAGMENT_FLAG, fragmentList.get(EXPLORE_FRAGMENT_FLAG));
                    isExploreFragmentLoaded = true;
                }
                break;
            case ADD_FRAGMENT_FLAG:
                if (!isAddFragmentLoaded) {
                    loadMultipleRootFragment(R.id.fl_main, ADD_FRAGMENT_FLAG, fragmentList.get(ADD_FRAGMENT_FLAG));
                    isAddFragmentLoaded = true;
                }
                break;
            case MESSAGE_FRAGMENT_FLAG:
                if (!isMessageFragmentLoaded) {
                    loadMultipleRootFragment(R.id.fl_main, MESSAGE_FRAGMENT_FLAG, fragmentList.get(MESSAGE_FRAGMENT_FLAG));
                    isMessageFragmentLoaded = true;
                }
                break;
            case USER_FRAGMENT_FLAG:
                if (!isPersonalCenterFragmentLoaded) {
                    loadMultipleRootFragment(R.id.fl_main, USER_FRAGMENT_FLAG, fragmentList.get(USER_FRAGMENT_FLAG));
